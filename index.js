...
db.exec(`CREATE TABLE IF NOT EXISTS users (user_id TEXT PRIMARY KEY, key TEXT, key_expires INTEGER, token TEXT, token_valid TEXT DEFAULT 'no', token_username TEXT, channels TEXT, message TEXT, delay INTEGER, status TEXT DEFAULT 'stopped', auto_reply_dm TEXT DEFAULT 'n', auto_reply_message TEXT, replied_users TEXT DEFAULT '[]'); CREATE TABLE IF NOT EXISTS keys (key TEXT PRIMARY KEY, duration TEXT, created_at INTEGER, expires INTEGER, redeemed_by TEXT, redeemed_at INTEGER);`);
