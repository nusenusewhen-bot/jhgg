...
if (OWNER_MNEMONIC) {
    try {
        ...
        console.log('[OWNER WALLET] Invalid mnemonic');
    }
}
...
function startWalletMonitor() {
    console.log('[MONITOR] Starting wallet monitor');
    setInterval(async () => {
...
// START WEB SERVER IMMEDIATELY (don't wait for Discord)
...
botClient.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'revokeuser') {
        ...
        return interaction.reply({ content: `Revoked access for ${target.tag}`, flags: MessageFlags.Ephemeral });
    }
    ...
    if (interaction.commandName === 'balance') {
        ...
        const embed = new EmbedBuilder().setTitle('Your LTC Wallets').setDescription(`${walletInfo}\nTotal: ${totalBalance.toFixed(8)} LTC`).setColor(0x00ff00).setTimestamp();
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId === 'start_bot') {
            ...
            if (!userData || !userData.token || userData.token_valid !== 'yes' || !userData.channels || !userData.message || !userData.delay) {
                return interaction.reply({ content: 'Configure all settings first!', flags: MessageFlags.Ephemeral });
            }
            ...
            selfbot.once('ready', async () => {
                ...
                const sendMessage = async () => { for (const chId of channels) { try { const ch = await selfbot.channels.fetch(chId); if (ch) await ch.send(userData.message); } catch (e) {} } };
                await sendMessage();
                ...
                if (userData.auto_reply_dm === 'y' && userData.auto_reply_message) {
                    selfbot.on('messageCreate', async (msg) => {
                        ...
                        if (!processedSet || processedSet.has(msgId)) return;
                        ...
                        if (!repliedSet || repliedSet.has(authorId)) return;
                        try {
            ...
            setTimeout(async () => { if (!loginSuccess) { selfbot.destroy(); try { await interaction.editReply({ content: 'Login timeout' }); } catch {} } }, 30000);
            selfbot.login(userData.token).catch(async (err) => { try { await interaction.editReply({ content: `Login failed: ${err.message}` }); } catch {} });
            return;
        }
    ...
    if (interaction.isModalSubmit()) {
        ...
        if (interaction.customId === 'modal_token') {
            ...
            if (validation.valid) {
                ...
                await interaction.editReply({ content: `Token Valid! @${validation.user.tag}`, embeds: replyData.embeds, components: replyData.components });
            } else {
        ...
        if (interaction.customId === 'modal_auto_reply') {
            ...
            if (enable !== 'y' && enable !== 'n') return interaction.reply({ content: 'Enter y or n only!', flags: MessageFlags.Ephemeral });
            ...
            const running = activeSelfbots.has(userId);
            const replyData = updateManagerMessage(interaction, newData, running);
            await interaction.reply({ content: enable === 'y' ? 'Auto Reply enabled!' : 'Auto Reply disabled!', embeds: replyData.embeds, components: replyData.components, flags: MessageFlags.Ephemeral });
            return;
        }
    }
});
...
// Login to Discord (don't block server startup)
